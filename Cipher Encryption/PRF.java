...
public class PRF implements Proj1Constants {
	...
	// It produces a result of size <PRFOutputSizeBits> bits == <PRFOutputSizeBytes> bytes.
	...
	// - Also, this is the ONLY crypto code you are allowed to use,
	...
	public synchronized void eval(byte[] inBuf, int inOffset, int numBytes,
		...
		//   through outBuf[outOffset+PRFOutputSizeBytes-1].
	...
	public synchronized byte[] eval(byte[] inBuf, int inOffset, int numBytes) {
		// Provide final input to the PRF, and get its output.  The input value
		//   is inBuf[inOffset] through inBuf[inOffset+numBytes-1].
		// This returns a byte-array of length <PRFOutputSizeBytes>.
		try {
			byte[] ret = new byte[PRFOutputSizeBytes];
	...
	public static void main(String[] argv) {
		...
		PRF prf = new PRF(k);
		byte[] x = prf.eval(v);
		assert x.length == PRFOutputSizeBytes;
		byte[] x2 = prf.eval(v2);
		assert x2.length == PRFOutputSizeBytes;
		assert !(x.equals(x2));

		PRF prf2 = new PRF(k);
		byte[] xAgain = prf2.eval(v);
		assert xAgain.length == PRFOutputSizeBytes;
